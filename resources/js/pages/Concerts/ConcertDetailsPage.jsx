...
export default function ConcertDetailsPage() {
  ...
  const handleBookTicket = (showtimeId) => {
    ...
    navigate(`/concerts/${id}/seats?showtime_id=${showtimeId}`);
  };
  ...
  useEffect(() => {
    const fetchConcertData = async () => {
      try {
        ...
        // Fetch showtimes
        const stRes = await fetch(`/api/showtimes?event_id=${id}&event_type=concert`);
    ...
    <div className="relative w-full bg-[#0D0D0D] min-h-screen font-mulish text-white">
      ...
      <section className="relative w-full h-[50vh] md:h-[60vh] overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `url(${concert?.poster_url || "/images/defaults/concert.png"})` }}
        />
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="flex-1">
              ...
              <p className="text-gray-300 text-sm md:text-lg max-w-2xl leading-relaxed line-clamp-3 md:line-clamp-none">
                {concert?.description || `${concert?.title} – Live performance.`}
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 border-b border-white/5 pb-8">
            <div>
              ...
              <p className="text-gray-400 text-sm md:text-base">
                Select your preferred date and venue to continue booking.
              </p>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/5">
              <Calendar size={18} className="text-purple-400" />
              <span className="text-sm font-medium">Multiple Dates Available</span>
            </div>
          </div>
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-12 flex-1">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-bold md:text-lg">{st.venue?.name || concert?.location || '—'}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 md:gap-10 pt-4 md:pt-0 border-t md:border-t-0 border-white/5">
                    <div className="flex flex-col items-end">
                      <span className="text-xs text-gray-400 uppercase tracking-wider mb-1 text-right">Starting from</span>
