...
export default function ConcertsPage() {
  ...
  const fetchConcerts = async (isNewSearch = false) => {
    if (isNewSearch) {
      setIsLoading(true);
      setPage(1);
    } else {
    ...
    try {
      ...
      setConcerts(prev => isNewSearch ? newItems : [...prev, ...newItems]);
  ...
  const handleViewAll = () => {
    ...
    setIsLoading(true);
    setPage(1);
    getConcerts(resetParams).then(res => {
      setConcerts(res.data.data || res.data);
  ...
  if (isLoading && page === 1) {
    return (
      <div className="loading-center">
        ...
        <p className="loading-text">{t('concerts.page.loading')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full bg-[#0D0D0D] mt-[80px] md:mt-[158px] min-h-screen">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {concerts.map((concert, index) => (
              <Link
                to={`${adminPath}/concerts/${concert.id}`}
                key={concert.id}
                ref={index === concerts.length - 1 ? lastElementRef : null}
              ...
              >
                ...
                <div className="p-3 md:p-4 flex flex-col gap-1 md:gap-2">
                  <h2 className="font-poppins text-sm md:text-lg font-bold text-white line-clamp-1">
                    {concert.performer || concert.title}
                  </h2>
                  <div className="flex items-center gap-1 md:gap-2 text-[#999999] text-[10px] md:text-sm">
                    <Calendar className="w-3 h-3 md:w-4 md:h-4 flex-shrink-0" />
                    <span>{formatDateRange(concert.first_show_date, concert.last_show_date)}</span>
                  </div>
                  <div className="flex items-center gap-1 md:gap-2 text-[#999999] text-[10px] md:text-sm">
      ...
      <section className="relative w-full h-[263px] my-[60px] md:my-[120px] flex justify-start items-center">
