...
export default function MovieDetailsPage() {
  ...
  const handleBookTicket = (showtimeId) => {
    ...
    navigate(`/movies/${id}/seats?showtime_id=${showtimeId}`);
  };
  ...
  useEffect(() => {
    const fetchMovieData = async () => {
      try {
        ...
        // Fetch showtimes
        const stRes = await fetch(`/api/showtimes?event_id=${id}&event_type=movie`);
    ...
    <div className="relative w-full bg-[#0D0D0D] min-h-screen font-mulish text-white">
      ...
      <section className="relative w-full h-[50vh] md:h-[60vh] overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `url(${movie?.poster_url || "/images/defaults/movie.png"})` }}
        />
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="flex items-center gap-1.5 text-gray-300 text-sm">
                  ...
                  <span>{movie?.location || t('movies.page.cinema')}</span>
                </div>
              </div>
              ...
              <p className="text-gray-300 text-sm md:text-lg max-w-2xl leading-relaxed line-clamp-3 md:line-clamp-none">
                {movie?.description || `${movie?.title} – A cinematic experience you won't forget.`}
              </p>
            </div>
            
            <div className="hidden md:block w-48 h-[270px] rounded-2xl overflow-hidden shadow-2xl border-2 border-white/10 rotate-3 hover:rotate-0 transition-transform duration-500 flex-shrink-0">
              <img 
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 border-b border-white/5 pb-8">
            <div>
              ...
              <p className="text-gray-400 text-sm md:text-base">
                Check the schedule and pick a time that works for you.
              </p>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/5">
              <Calendar size={18} className="text-purple-400" />
              <span className="text-sm font-medium">Daily Screenings</span>
            </div>
          </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-12 flex-1">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs text-gray-400 uppercase tracking-wider mb-1">Cinema</span>
                        <span className="font-bold md:text-lg">{st.venue?.name || movie?.location || t('movies.page.cinema')}</span>
                      </div>
                    </div>
                    
                    <div className="flex flex-col">
                      <span className="text-xs text-gray-400 uppercase tracking-wider mb-1">Session Time</span>
                      <span className="font-semibold text-gray-200">
                        {formatDateTime(st.start_time)}
                        {movie?.duration != null && <span className="text-gray-500 ml-2 font-normal">({movie.duration} min)</span>}
                      </span>
                    </div>
                  </div>
