...
export default function StandupsPage() {
  ...
  const fetchStandups = async (isNewSearch = false) => {
    if (isNewSearch) {
      setIsLoading(true);
      setPage(1);
    } else {
    ...
    try {
      const response = await getStandups(params);
      const newItems = response.data.data || response.data; // Handle both paginated and non-paginated responses
      
      setStandups(prev => isNewSearch ? newItems : [...prev, ...newItems]);
  ...
  const handleViewAll = () => {
    ...
    setFilterPrice("");
    // We need to wait for state updates or pass params directly
    const resetParams = { when: "all", where: "all", page: 1 };
    setIsLoading(true);
    setPage(1);
    getStandups(resetParams).then(res => {
      setStandups(res.data.data || res.data);
  ...
  if (isLoading && page === 1) {
    return (
      <div className="loading-center">
        ...
        <p className="loading-text">{t('standups.page.loading')}</p>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {standups.map((standup, index) => (
              <Link
                ...
                key={standup.id}
                ref={index === standups.length - 1 ? lastElementRef : null}
              ...
              >
                ...
                <div className="p-3 md:p-4 flex flex-col gap-1 md:gap-2">
                  ...
                  <div className="flex items-center gap-1 md:gap-2 text-[#999999] text-[10px] md:text-sm">
                    ...
                    <span>{formatDateRange(standup.first_show_date, standup.last_show_date)}</span>
                  </div>
                  <div className="flex items-center gap-1 md:gap-2 text-[#999999] text-[10px] md:text-sm">
