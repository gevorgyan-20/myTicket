...
export default function ConcertCreatePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setForm({ title: '', description: '', performer: '', start_time: '', end_time: '', location: '', poster_url: '', venue_id: '' });
            setVenueType('');
        } catch (err) {
        ...
        <div className="min-h-screen bg-[#0c0c0c] px-6 py-12">
            <div className="max-w-2xl mx-auto">
                <h1 className="text-3xl font-bold text-white mb-2">
                    <span className="text-purple-400">🎵</span> Create New Concert
                </h1>
                <p className="text-gray-400 mb-8">Add a new concert to the system</p>
                ...
                <form onSubmit={handleSubmit} className="bg-[#1a1a2e] border border-purple-900/30 rounded-2xl p-8 space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value={form.title} onChange={handleChange} required placeholder="Concert title" className={inputClass} />
                    </div>

                    <div>
                        <label className={labelClass}>Description</label>
                        <textarea name="description" value={form.description} onChange={handleChange} rows="3" placeholder="Concert description" className={inputClass + " resize-none"} />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelClass}>Performer <span className="text-red-400">*</span></label>
                            <input type="text" name="performer" value={form.performer} onChange={handleChange} required placeholder="Artist name" className={inputClass} />
                        </div>
                        <div>
