...
export default function MovieCreatePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setForm({ title: '', description: '', genre: '', duration: '', release_date: '', poster_url: '', venue_id: '' });
            setVenueType('');
        } catch (err) {
        ...
        <div className="min-h-screen bg-[#0c0c0c] px-6 py-12">
            <div className="max-w-2xl mx-auto">
                <h1 className="text-3xl font-bold text-white mb-2">
                    <span className="text-purple-400">🎬</span> Create New Movie
                </h1>
                <p className="text-gray-400 mb-8">Add a new movie to the system</p>
                ...
                <form onSubmit={handleSubmit} className="bg-[#1a1a2e] border border-purple-900/30 rounded-2xl p-8 space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value={form.title} onChange={handleChange} required placeholder="Movie title" className={inputClass} />
                    </div>

                    <div>
                        <label className={labelClass}>Description</label>
                        <textarea name="description" value={form.description} onChange={handleChange} rows="3" placeholder="Movie description" className={inputClass + " resize-none"} />
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelClass}>Genre</label>
                            <input type="text" name="genre" value={form.genre} onChange={handleChange} placeholder="e.g. Action" className={inputClass} />
                        </div>
                        <div>
                            <label className={labelClass}>Duration (min) <span className="text-red-400">*</span></label>
                            <input type="number" name="duration" value={form.duration} onChange={handleChange} required min="1" placeholder="120" className={inputClass} />
                        </div>
                    </div>
